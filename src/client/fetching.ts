...
import { AuthorHit, DblpSearchResult, RawDblpBaseSearchResult, VenueHit } from '@/dtos/DblpSearchResult'
...
function createSearchFetcher<HitT>() {
    return (input: string) => {
        if (!input.includes('q='))
            return null;

        return fetch(input).then(res =>
            res.json().then(data => new DblpSearchResult<HitT>(data as RawDblpBaseSearchResult)))
    };
}

const authorsSearchFetcher: Fetcher<DblpSearchResult<AuthorHit> | null, string> = createSearchFetcher<AuthorHit>();
const venuesSearchFetcher: Fetcher<DblpSearchResult<VenueHit> | null, string> = createSearchFetcher<VenueHit>();
