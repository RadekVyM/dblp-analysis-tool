...
export default function DividedDefinitionList({ items, className }: DividedDefinitionListParams) {
    return (
        <dl
            className={cn('flex flex-col xs:flex-row xs:items-center text-sm gap-x-1 gap-y-2', className)}>
            {items.map((item, index) =>
                <div key={item.term} className='flex flex-col xs:flex-row xs:items-center text-sm gap-1'>
                    {
                        index !== 0 &&
